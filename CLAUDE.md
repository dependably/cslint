...
cslint is a .NET 10 global CLI tool (multi-targeted `net8.0;net10.0`) that lints C# files across four tiers: EditorConfig enforcement, SAST (security), Roslyn semantic analysis, and opinionated pattern scans. Per-rule behavior is driven by `.editorconfig` (`dotnet_diagnostic.<RuleId>.severity`); suite-wide settings (the CI gate's `strict`, the `scan` toggles, exclude globs) come from the shared repo-root `.dependably-check` JSON (`CsLintConfig`). Quantitative code metrics are a separate concern, owned by the companion `codemetrics` tool.
...
**CI gate (`--fail-on`):** The single suite-wide gate is `--fail-on <key>=<value>` (repeatable; parsed in `Cli.ParseFailOn` into `FailOnRule` records). `severity=<critical|high|moderate|low|info>` trips when any finding is at-or-above the level (cslint ranks `error`=high=3, `warning`=low=1; the raw words `error`/`warning` are accepted aliases); `count=<N>` trips when total findings exceed N. A bad key/value sets `CliOptions.OptionError` → exit 2. When no `--fail-on` is given, `ApplyConfig` installs the default (errors gate) plus a warning-gate when `.dependably-check` sets `strict: true`; a CLI `--fail-on` replaces that default outright. `Cli.GateTripped` evaluates the rules; a trip is exit 1. This replaced the former `--strict`/`-s` flag (`--fail-on severity=warning` reproduces it). The opinionated rule-toggle flags `--no-magic-numbers`/`--no-bool-flags`/`--no-cancellation` were also removed — OP004/005/006 are disabled via `.editorconfig` `severity = none` or the `.dependably-check` `scan` toggles.
